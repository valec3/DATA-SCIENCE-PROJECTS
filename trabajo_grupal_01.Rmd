...
author: "Victor, Rene, Noe, Jhon, Julio"
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
...
- Machaca Chambilla Noe Ulises 
- Gonzales Quispe Jhon Cristhian 
- Ccoyto Pachapuma Rene Rolando
...
ggplot(datos, aes(x = "", fill = booking_status)) +
  geom_bar(width = 1) +
  coord_polar("y", start = 0) +
  labs(title = "Proporción de Estado de Reservas") +
  theme_void() +
...
## Analisis temporal 
```{r}
# Crear columna de fecha y agrupar por mes
df_monthly <- datos %>%
  mutate(arrival_date_full = make_date(arrival_year, arrival_month, arrival_date),
         month = floor_date(arrival_date_full, "month")) %>%
  group_by(month) %>%
  summarise(total_bookings = n())

# Gráfico por meses con línea suavizada
ggplot(df_monthly, aes(x = month, y = total_bookings)) +
  geom_line(color = "steelblue", size = 1) +
  geom_point(color = "darkred", size = 2) +
  geom_smooth(method = "loess", se = FALSE, 
              color = "orange", size = 1.2, linetype = "dashed") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b-%Y") +  # todos los meses
  labs(title = "Reservas por mes con tendencia suavizada",
...
# GRAFICA
ggplot(lealtad_analysis, aes(x = as.factor(repeated_guest), y = prop, fill = booking_status)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "TASA DE CANCELACIÓN POR TIPO DE CLIENTE",
       subtitle = "Clientes recurrentes muestran tasas de cancelación dramáticamente más bajas",
       x = "Cliente Recurrente (0=No, 1=Sí)", 
       y = "Proporción (%)") +
  theme_minimal()
